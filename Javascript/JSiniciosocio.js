...
function iniciar() {
    ...
    document.getElementById("titulonombreasociado").innerHTML = '<h3 style="color: black;">' + JSON.parse(localStorage.getItem("ASOCIADOLOGUEADO")).IDENTIFICACION + ' - ' + JSON.parse(localStorage.getItem("ASOCIADOLOGUEADO")).NOMBRE + ' ' + JSON.parse(localStorage.getItem("ASOCIADOLOGUEADO")).PRIMER_APELLIDO + ' ' + JSON.parse(localStorage.getItem("ASOCIADOLOGUEADO")).SEGUNDO_APELLIDO + '</h3>';
    ...
    document.getElementById("espaciodatospersonales").innerHTML = "</br><br/><br/></br><br/><br/></br><br/><br/></br><br/><br/>Fecha de nacimiento: " + JSON.parse(localStorage.getItem("ASOCIADOLOGUEADO")).FECHA_NACIMIENTO + " </br><br/>Dirección: " + JSON.parse(localStorage.getItem("ASOCIADOLOGUEADO")).DIRECCION + "</br><br/>Teléfono(s): " +
        ...
        JSON.parse(localStorage.getItem("ASOCIADOLOGUEADO")).EMAIL2 +
        "</br><br/>Fecha de ingreso al ICE: " + JSON.parse(localStorage.getItem("ASOCIADOLOGUEADO")).FECHA_INGRESO_ICE + '<hr><p><strong> Datos facilitados por el departamento de recursos humanos del ICE</strong></p>';
    ...
    /*LAS PETICIONES AJAX DEL SISTEMA SE REALIZAN A LOS CONTROLADORES DE LA CARPETA CONTROLLERS, SIEMPRE UTILIZANDO EL VERBO POST, SE UTILIZA JQUERY PARA EL USO DE AJAX*/$.ajax({
        ...
        /*CUANDO EL ERVIDOR ENVIA UNA RESPUESTA AFIRMATIVA*/success: function (data, textStatus, xhr) {
            if (data.IDENTIFICACION.IDENTIFICACION == 0) {
                $("#espaciodatosmutualidad").append("Persona no esta afiliada a PBDA");
                ...
                $("#espaciodatosmutualidad").append("<h2>PROGRAMA BENEFICIO POR DEFUNCIÓN DEL AFILIADO (PBDA)</h2><br><img id=\"imgboletamut\" src=\"Boletas/" + data.IDENTIFICACION.IDENTIFICACION + "mut.jpg\" onerror=\"this.src=\'Imagenes/imgboletano.jpg'\"><br><p style='color:black;'> Estado actual en el fondo: " + data.ESTADO.DESCRIPCION + " <br> Forma de pago: " + data.METODO_PAGO.DESCRIPCION + "<br> Fecha de afiliación: " + data.FECHA_INGRESO + "<br> Saldo actual: " + data.MONTO + " </spam> </strong> </p><hr><paper-button raised  id='botonpopbene'  onclick='muestradivbn()'>Mostrar beneficiarios</paper-button> <paper-button raised  id='botonpophistorialmut' onclick=' muestradivhistorial(\"mutualidad\")' >Mostrar historial de pagos</paper-button> ");
    ...
    /*LAS PETICIONES AJAX DEL SISTEMA SE REALIZAN A LOS CONTROLADORES DE LA CARPETA CONTROLLERS, SIEMPRE UTILIZANDO EL VERBO POST, SE UTILIZA JQUERY PARA EL USO DE AJAX*/$.ajax({
        ...
        /*CUANDO EL ERVIDOR ENVIA UNA RESPUESTA AFIRMATIVA*/success: function (data, textStatus, xhr) {
            if (data.IDENTIFICACION.IDENTIFICACION == 0) {
                ...
                $("#espaciodatosasobiso").append("<h2>Asobiso</h2><br><p style='color:black;'> Estado actual en el fondo: " + data.ESTADO.DESCRIPCION + "<br> Forma de pago: " + data.METODO_PAGO.DESCRIPCION + " <br> Fecha de afiliación: " + data.FECHA_INGRESO + "<br> Saldo actual: " + data.MONTO + " </spam> </strong> </p>");
...
function cargasol(iden) {
    ...
    /*LAS PETICIONES AJAX DEL SISTEMA SE REALIZAN A LOS CONTROLADORES DE LA CARPETA CONTROLLERS, SIEMPRE UTILIZANDO EL VERBO POST, SE UTILIZA JQUERY PARA EL USO DE AJAX*/$.ajax({
        ...
        /*CUANDO EL ERVIDOR ENVIA UNA RESPUESTA AFIRMATIVA*/success: function (data, textStatus, xhr) {
            if (data.IDENTIFICACION.IDENTIFICACION == 0) {
                $("#espaciodatossolidaridad").append("Persona no esta afiliada a PBDF");

            } else {

                $("#espaciodatossolidaridad").append("<h2>PROGRAMA BENEFICIO POR DEFUNCIÓN DE FAMILIARES (PBDF)</h2><img id=\"imgboletasol\" src=\"Boletas/" + data.IDENTIFICACION.IDENTIFICACION + "sol.jpg\" onerror=\"this.src=\'Imagenes/imgboletano.jpg'\"><br><p style='color:black;'> Estado actual en el fondo: " + data.ESTADO.DESCRIPCION + "<br> Forma de pago: " + data.METODO_PAGO.DESCRIPCION + " <br> Fecha de afiliación: " + data.FECHA_INGRESO + " <br> Saldo actual: " + data.MONTO + " </spam> </strong> </p><hr> <paper-button raised  id='botonpopfam'  onclick='muestradivfam()'>Mostrar familiares</paper-button><paper-button raised  id='botonpophistorialsol' onclick='muestradivhps(\"solidaridad\")'>Mostrar historial de pagos</paper-button>  ");
            ...
            if (data.ID == 0) {
              ...
              /*ITERAMOS EL VECTOR DE DATOS RECUPERADO DESDE EL SERVIDOR*/  for (var ele in data) {
                    ...
                    if (!/^([0-9])*$/.test(data[ele].IDENTIFICACION_BENEFICIARIO.IDENTIFICACION)) {
                        var newText = document.createTextNode(data[ele].IDENTIFICACION_BENEFICIARIO.IDENTIFICACION + ' Es posible que el número sea incorrecto')
                ...
                $('#benficiariospopup').append("<br><strong>Si alguna identificación tiene letras, significa que no tenemos claro ese número, favor indicarnos el número correcto por medio de la sección \"Contacto\"</strong>");
    ...
    /*LAS PETICIONES AJAX DEL SISTEMA SE REALIZAN A LOS CONTROLADORES DE LA CARPETA CONTROLLERS, SIEMPRE UTILIZANDO EL VERBO POST, SE UTILIZA JQUERY PARA EL USO DE AJAX*/$.ajax({
        ...
        /*CUANDO EL ERVIDOR ENVIA UNA RESPUESTA AFIRMATIVA*/success: function (data, textStatus, xhr) {
            if (data.ID == 0) {
                ...
                $('#familiarespopup').append("<h1>Familiares</h1><table id='tablafamiliares' class='table2'><thead><tr><th scope='col' abbr='Starter'>Identificación del familiar</th><th scope='col' abbr='Starter'>Nombre del familiar</th><th scope='col' abbr='Starter'>Estado</th><th scope='col' abbr='Starter'>Parentezco</th></tr></thead><tbody>");
...
function cerrarsesionysalir() {
   /*GUARDADO EN MEMORIA LOCAL, COOKIE EL DATO*/ localStorage.setItem("ASOCIADOLOGUEADO", null);
    location.href = "/loginsocio.html";
...
function muestradivconfig() {
    ...
    if (document.getElementById("configuracion").style.visibility == 'visible') {
        ...
        document.getElementById("btnconfigura").innerHTML = 'Configuración';
    } else {
        document.getElementById("configuracion").style.visibility = 'visible';
        document.getElementById('configuracion').style.height = '100%';
        document.getElementById("btnconfigura").innerHTML = 'Ocultar configuración';
    }
}
